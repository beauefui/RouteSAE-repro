...
本节展示了基于 10,000 个 OpenWebText 样本的实际评估结果，并结合论文理论进行了深入分析。这些数据直观地展示了不同 SAE 架构在“重构能力”与“功能保持”之间的权衡。
...
> **指标解读**:
> *   **NormMSE (Normalized Mean Squared Error)**: 衡量 SAE 还原原始激活值的能力。
>     *   值越低，说明重构越精准。
>     *   但过低的 NormMSE 可能意味着模型放弃了稀疏性，直接复制输入 (Identity Mapping)。
> *   **KL Divergence (Kullback-Leibler Divergence)**: 衡量 SAE 替换后对 LLM 下游预测分布的影响。
>     *   值越低，说明 SAE 提取的特征越好地保留了原始模型的语义功能。
>     *   这是比 NormMSE 更关键的指标，因为我们最终关心的是特征的“语义有效性”。
