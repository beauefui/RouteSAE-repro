...
本节展示了基于 10,000 个 OpenWebText 样本的实际评估结果，并结合论文理论进行了深入分析。
...
> **注**:
> *   **NormMSE (重构误差)**: 越低越好。反映模型“还原”原始激活的能力。
> *   **KL Divergence (功能损耗)**: 越低越好。反映 SAE 替换后对 LLM 下游预测分布的影响。

### 2. 深度对比分析

#### 2.1 基线验证 (Baseline Validation)
*   **RandomK**: 其 NormMSE 高达 0.86，KLDiv 高达 3672。这作为一个“最差下界”，有力地证明了其他所有训练过的 SAE 模型（NormMSE < 0.3）并非是在做随机映射，而是确实学到了有意义的特征结构。

#### 2.2 核心对比: RouteSAE vs TopK
这是验证 RouteSAE 论文核心贡献的关键对比：
*   **现象**: RouteSAE 的重构误差 (0.21) 略高于 TopK (0.19)，但在下游任务指标 KL Divergence 上，**RouteSAE (124.58) 显著优于 TopK (371.16)**，性能提升了近 3 倍。
*   **结论**: 这证明了 **“路由机制” (Routing Mechanism)** 的有效性。通过动态选择最适合的层（Layer 4-12）进行特征提取，RouteSAE 捕捉到了更能保持模型语义功能的特征，而不仅仅是在单层上做死记硬背的像素级重构。

#### 2.3 异常值分析: Vanilla & Gated
*   **现象**: Vanilla 和 Gated 模型展现了异常低的重构误差 (~0.008) 和极佳的 KLDiv (~14)。
*   **风险提示**: 在盲目庆祝之前，需要警惕 **“恒等映射” (Identity Mapping)** 风险。如此低的误差通常意味着模型可能没有足够的稀疏性 (即 L0 很高)，它可能学到了直接复制输入。对于追求“稀疏可解释性”的任务来说，这可能是一个过拟合的信号，建议在后续实验中增大 L1 正则系数。

#### 2.4 多层模型对比: RouteSAE vs MLSAE
*   **MLSAE**: 试图用统一的特征空间解释所有层，其 KLDiv (232.46) 优于单层 TopK，但不如 RouteSAE。
*   **结论**: 这表明 LLM 的不同层之间存在显著的 **“语义漂移” (Semantic Drift)**。RouteSAE 的分层/路由策略比 MLSAE 的强制统一策略更符合 LLM 的内部特性。

### 3. final 结论

1.  **首选模型**: **RouteSAE** 是平衡稀疏性、重构质量与下游任务性能的最佳选择。
2.  **基线模型**: TopK 依然是简单可靠的单层分析工具。
3.  **Crosscoder**: 展示了跨层编码的潜力，但目前实现主要关注重构，未来可进一步探索其功能性影响。
